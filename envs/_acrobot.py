...
class Acrobot(gym.Env):
    ...
    **REFERENCE:**
    ...
    def __init__(self, seed=0, horizon=50):
        high = np.array([1.0, 1.0, self.MAX_VEL_1, self.MAX_VEL_2], dtype=np.float32)
        low = -high
    ...
    def step(self, action):
        torque = bound(action, -2.0, 2.0)  # continuous action space
        self.last_u = action
        # Add noise to the force action
        # if self.torque_noise_max > 0:
        #    torque += self.np_random.uniform(-self.torque_noise_max, self.torque_noise_max)
        reward = self.reward_fn(self.state, action)
        next_state = self.dynamics(self.state, torque)
        a, b, c, d = next_state
        a = wrap(a, -np.pi, np.pi)
        b = wrap(b, -np.pi, np.pi)
        c = bound(c, -self.MAX_VEL_1, self.MAX_VEL_1)
        d = bound(c, -self.MAX_VEL_2, self.MAX_VEL_2)

        self.state = jnp.array([a, b, c, d])
...
def rk4(derivs, y0, t, *args, **kwargs):
    ...
    try:
        ...
        yout = jnp.zeros((len(t),), jnp.float32)
    else:
        yout = jnp.zeros((len(t), Ny), jnp.float32)
...
def _test_acrobot():
    ...
    s = env.reset()
    s1, r, d, info = env.step(jnp.ones(1, ))
    print(r)
