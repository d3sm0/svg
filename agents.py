...
class SVG:
    ...
    def _optimize_model(self, batch):
        ...
        for (states, rewards, v_t, pi_env) in train_slice:
            ...
            rho_tm1 = torch.exp(pi_true.log_prob(a_tm1) - pi_env.log_prob(a_tm1)).sum(dim=-1).clamp_max(2.)
            v_trace_output = rlego.vtrace_td_error_and_advantage(v_tm1.squeeze(dim=-1), v_t, rewards, discount_t,
                                                                 rho_tm1.detach()
                                                                 )
            pi_loss = - (rho_tm1 * v_trace_output.pg_advantage * (1 - config.gamma)).sum()
            ...
            total_loss = total_loss + (value_loss + reward_loss + q_loss + kl - pi_loss)
