...
def unroll(dynamics, policy, traj, state):
    ...
    for t, transition in enumerate(traj):
        ...
        if torch.rand(1, ).item() < config.detach:
...
def train(dynamics, policy, pi_optim, trajectory):
    ...
    for _ in range(config.opt_epochs):
        ...
        model_norm = get_grad_norm(parameters=dynamics.parameters())
        assert torch.isfinite(true_norm)
    ...
    return {"agent/return": total_return, "agent/td_error": total_td, "agent/grad_norm": true_norm,
            "agent/dynamic_norm": model_norm, **extra}
